...
<div class="theorem">
  ...
  <details>
  <summary>Proof.</summary>
  See Newey and McFadden<span markdown="1">[^fn-newey]</span> pg. 2121 for a proof. 
  </details>
</div>

<details>
<summary>Relaxations.</summary>
...
$$
Q_0(X, \theta') < Q_0(X, \theta) + \epsilon \hspace{2mm} \forall \theta' \in \mathcal{B}
$$
The fourth condition can be changed to just require that:
$$
\hat{Q}_n(X, \theta_0) \overset{p}{\rightarrow} Q_0(\theta_0), 
\hspace{5mm} \text{and} \hspace{5mm}
\hat{Q}_n(X, \theta) < Q_0(X, \theta) + \epsilon \hspace{2mm} \forall \epsilon > 0, \hspace{1mm} \forall \theta \in \Omega \hspace{2mm} \text{with probability approaching one}
$$
If we make the stronger assumption that:
$$
...
\leq a^\top \mathbb{E}_{\theta_0}[\psi^{(2)}(X, \theta_0) (\psi^{(2)}(X, \theta_0))^\top]a , \hspace{10mm} \forall q \times 1 \text{ constant vectors } a
$$
which itself is equivalent to:
$$
a^\top \left[ \mathbb{E}_{\theta_0}[\psi^{(2)}(X, \theta_0) (\psi^{(2)}(X, \theta_0))^\top] - \mathbb{E}_{\theta_0}[\psi^{(1)}(X, \theta_0) (\psi^{(1)}(X, \theta_0))^\top] \right] a \geq 0, \hspace{10mm} \forall q \times 1 \text{ constant vectors } a
$$
</details>
